...
>
> 여기서 멀티모달 센서 융합을 사용하는 이유는 단일 센서의 한계를 극복하기 위해서입니다. 카메라만 사용하면 야간이나 역광에 취약하고, LiDAR만 사용하면 색상이나 표정 인식이 불가능합니다. 하지만 이들을 융합하면 낮에는 RGB를, 밤에는 열화상을 활용하여 24시간 안정적으로 작동합니다.
>
...
>
> 이 시스템이 실제로 어디에 사용되는지 좀 더 구체적으로 말씀드리겠습니다.
>
> 발전소나 공항 같은 중요시설의 야간 무인 순찰이 대표적입니다. 4족 로봇이 자율적으로 순찰하면서 침입자를 탐지하고, 저조도 환경에서도 열화상 카메라로 사람을 추적합니다.
>
> 또한 3D Gaussian Splatting으로 재구성된 환경은 PSNR 27dB 이상의 품질을 보이는데, 이는 실제 사진과 거의 구분이 안 될 정도의 수준입니다.
>
> 특히 중요한 것은 Real2Sim 파이프라인입니다. 실제 환경을 스캔해서 Isaac Sim으로 변환하면, 로봇 AI를 위험 없이 시뮬레이션에서 먼저 학습시킬 수 있습니다. 이를 디지털 트윈이라고 부릅니다.
>
...
>
> 이 시스템이 실제로 얼마나 정확한지 보여드리겠습니다.
>
> 복잡한 환경에서 10명 이상이 동시에 움직여도, 각 사람마다 고유한 ID를 부여하고 지속적으로 추적합니다. 예를 들어 사람이 잠시 가려졌다가 다시 나타나도, 같은 ID를 유지하며 재인식합니다.
>
> 기술적으로는 Kalman Filter로 사람의 이동 경로를 예측하고, Hungarian Algorithm으로 프레임마다 최적의 ID를 매칭합니다.
>
> 실시간 시각화는 RViz2로 이루어지며, 3D 바운딩 박스와 33개 스켈레톤 랜드마크가 실시간으로 표시됩니다. 이를 통해 로봇이 사람의 위치뿐 아니라 자세와 동작까지 이해할 수 있습니다.
>
...
>
> 카페 무인 서비스 시스템으로, Triage, Order, Service, Conversation 에이전트가 협력하여 고객을 응대합니다. 단순히 명령을 수행하는 것이 아니라 맥락을 이해하고 능동적으로 응대합니다.
>
> 실제 AI Service Agent가 어떻게 작동하는지 시나리오를 보여드리겠습니다.
>
> 고객이 "아메리카노 2잔이랑 카페라떼 1잔 주세요"라고 말하면, Triage Agent가 이것이 주문 요청임을 파악하고 Order Agent로 전달합니다. Order Agent는 "아메리카노 2잔, 카페라떼 1잔 맞으시죠? 사이즈는 어떻게 하시겠어요?"라고 확인합니다.
>
> 만약 고객이 "아까 주문한 거 취소하고 싶어요"라고 하면, 맥락을 이해해서 "방금 주문하신 아메리카노 2잔, 카페라떼 1잔을 취소하시겠습니까?"라고 응답합니다. 단순 키워드 매칭이 아닌 진짜 맥락을 이해하는 것입니다.
>
> 또한 현재 기획 중인 무인 리테일 시스템도 이 기술을 활용합니다.
>
> 매장 입구와 천장에 설치된 멀티모달 센서가 고객과 직접 상호작용하며 메뉴를 제안하고, 주문을 접수하고, 위치를 추적합니다.
>
> 무인 리테일 시스템의 전체 고객 여정을 보면, 입장부터 퇴장까지 모든 과정이 자동화됩니다. 센서가 고객을 감지해서 인사하고, 자연어 주문을 받고, 3D로 좌석을 추적해서 "2번 테이블"을 자동으로 매칭합니다. 서빙 로봇이 정확한 테이블로 배달하고, 퇴장 시에는 분실물이나 이상 상황을 자동으로 감지해서 관리자에게 알립니다.
>
